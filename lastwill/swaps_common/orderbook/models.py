...
from email_messages import swaps_deploed_message, swaps_deploed_subject
...
def _get_memo():
    ...
    """
    return '0x' + ''.join(random.choice('abcdef'.join(digits)) for _ in range(64))
...
def _get_unique_link():
    ...
    """
    return ''.join(random.choice(ascii_lowercase.join(digits)) for _ in range(6))
...
class PublicActiveOrdersManager(Manager):
    ...
    def get_queryset(self):
        return super().get_queryset().filter(public=self.public,
...
class OrderBookSwaps(models.Model):
    ...
    unique_link = models.CharField(max_length=50, null=True, unique=True, default=_get_unique_link)
    ...
    swap_ether_contract = models.ForeignKey(Contract, null=True, on_delete=models.SET_NULL)
    is_exchange = models.BooleanField(default=False)
    notification = models.BooleanField(default=False)
    notification_email = models.CharField(max_length=50, null=True, default='')
    notification_telegram_name = models.CharField(max_length=50, null=True, default='')
    ...
    def msg_deployed(self):
        ...
        self.save()
        if self.user.email:
            swaps_link = '{protocol}://{url}/public-v3/{unique_link}'.format(protocol=SITE_PROTOCOL,
                                                                             unique_link=self.unique_link,
                                                                             url=SWAPS_URL)
            sendEMail(swaps_deploed_subject, swaps_deploed_message.format(swaps_link=swaps_link), [self.user.email])
