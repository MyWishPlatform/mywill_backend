...
from lastwill.contracts.models import (
    ...
    ContractDetailsNeoICO, ContractDetailsSolanaToken, ContractDetailsSWAPS, ContractDetailsSWAPS2,
    ContractDetailsToken, ContractDetailsTokenProtector, ContractDetailsTRONAirdrop, ContractDetailsTRONLostkey,
    ContractDetailsTRONToken, ContractDetailsWavesSTO, ContractDetailsXinFinToken, EOSAirdropAddress, EOSContract,
    EOSTokenHolder, EthContract, Heir, InvestAddress, InvestAddresses, NeoContract, SolanaContract, SolanaTokenInfo,
    TokenHolder, TRONContract, WhitelistAddress, send_in_queue)
from lastwill.parint import EthereumProvider
from lastwill.payments.api import create_payment
from lastwill.profile.models import *
from lastwill.rates.api import rate
from lastwill.settings import EMAIL_HOST_PASSWORD_SWAPS, EMAIL_HOST_USER_SWAPS
...
def count_sold_tokens(address):
    ...
    method_sign = '0x' + binascii.hexlify(int_to_big_endian(m_id('totalSupply', []))).decode()
    sold_tokens = eth_int.eth_call({
        'to': address,
        'data': method_sign,
    })
...
class ContractSerializer(serializers.ModelSerializer):
    ...
    def create(self, validated_data):
        ...
        validated_data['cost'] = Contract.get_details_model(contract_type).calc_cost(
            contract_details, validated_data['network'])
        ...
        if validated_data['user'].email:
            ...
            if contract.contract_type not in (11, 20, 21, 23):
                send_mail(email_messages.create_subject,
                          email_messages.create_message.format(network_name=network_name), DEFAULT_FROM_EMAIL,
                          [validated_data['user'].email])
            elif contract.contract_type in (20, 21):
                sendEMail(email_messages.swaps_subject, email_messages.swaps_message, validated_data['user'].email)
            elif contract.contract_type == 23:
                email = contract_details['email'] if contract_details['email'] else validated_data['user'].email
                send_mail(email_messages.protector_create_subject, email_messages.protector_create_text,
                          DEFAULT_FROM_EMAIL, [email])
            else:
                send_mail(email_messages.eos_create_subject,
                          email_messages.eos_create_message.format(network_name=network_name), DEFAULT_FROM_EMAIL,
                          [validated_data['user'].email])
        return contract
    ...
    def to_representation(self, contract):
        ...
        res['contract_details'] = self.get_details_serializer(
            contract.contract_type)(context=self.context).to_representation(contract.get_details())
        if contract.state != 'CREATED':
            usdt_cost = res['cost']
            if 'TESTNET' in contract.network.name or 'ROPSTEN' in contract.network.name:
                usdt_cost = 0
        else:
            usdt_cost = Contract.get_details_model(contract.contract_type).calc_cost(
                res['contract_details'], contract.network)
        ...
        if contract.network.name == 'EOS_MAINNET':
            res['cost']['EOS'] = str(
                Contract.get_details_model(contract.contract_type).calc_cost_eos(res['contract_details'],
                                                                                 contract.network))
            res['cost']['EOSISH'] = str(float(res['cost']['EOS']) * rate('EOS', 'EOSISH').value)
        if contract.network.name == 'EOS_TESTNET':
            res['cost']['EOS'] = 0
            res['cost']['EOSISH'] = 0
        if contract.network.name == 'TRON_MAINNET':
            res['cost']['TRX'] = str(
                Contract.get_details_model(contract.contract_type).calc_cost_tron(res['contract_details'],
                                                                                  contract.network))
        ...
        if contract.contract_type == 20:
            cost = Contract.get_details_model(contract.contract_type).calc_cost_usdt(
                res['contract_details'], contract.network) / NET_DECIMALS['USDT']
            res['cost'] = {
...
class TokenProtectorSerializer(serializers.ModelSerializer):
    ...
    class Meta:
        ...
        fields = [
            'owner_address', 'reserve_address', 'end_timestamp', 'email', 'with_oracle', 'oracle_inactive_interval',
            'white_label'
        ]

    def to_representation(self, contract_details):
        if contract_details.end_timestamp < timezone.now().timestamp(
        ) + 30 * 60 and contract_details.contract.state in ['CREATED', 'WAITING_FOR_PAYMENT']:
        ...
        for token in ApprovedToken.objects.filter(contract=contract_details):
            res['approved_tokens'].append({'address': token.address, 'is_confirmed': token.is_confirmed})
...
class ContractDetailsLastwillSerializer(serializers.ModelSerializer):
    ...
    class Meta:
        ...
        fields = ('user_address', 'active_to', 'check_interval', 'last_check', 'next_check', 'email', 'platform_alive',
...
class ContractDetailsLostKeySerializer(ContractDetailsLastwillSerializer):
    ...
    class Meta:
        ...
        fields = ('user_address', 'active_to', 'check_interval', 'last_check', 'next_check', 'white_label')
        extra_kwargs = {
            'last_check': {
                'read_only': True
            },
...
class ContractDetailsDelayedPaymentSerializer(serializers.ModelSerializer):
    ...
    class Meta:
        ...
        fields = ('user_address', 'date', 'recepient_address', 'recepient_email', 'white_label')
...
class ContractDetailsICOSerializer(serializers.ModelSerializer):
    ...
    class Meta:
        ...
        fields = ('soft_cap', 'hard_cap', 'token_name', 'token_short_name', 'is_transferable_at_once', 'start_date',
                  'stop_date', 'decimals', 'rate', 'admin_address', 'platform_as_admin', 'time_bonuses',
                  'amount_bonuses', 'continue_minting', 'cold_wallet_address', 'reused_token', 'token_type', 'min_wei',
                  'max_wei', 'allow_change_dates', 'whitelist', 'verification', 'verification_status',
                  'verification_date_payment', 'white_label')
        extra_kwargs = {
            'verification_status': {
                'read_only': True
            },
            'verification_date_payment': {
                'read_only': True
            },
        }
...
class ContractDetailsTokenSerializer(serializers.ModelSerializer):
    ...
    class Meta:
        ...
        fields = ('token_name', 'token_short_name', 'decimals', 'admin_address', 'token_type', 'future_minting',
                  'authio', 'authio_email', 'authio_date_payment', 'authio_date_getting', 'verification',
                  'verification_status', 'verification_date_payment', 'white_label')
        extra_kwargs = {
            'authio_date_payment': {
                'read_only': True
            },
...
class ContractDetailsNeoSerializer(serializers.ModelSerializer):
    ...
    class Meta:
        ...
        fields = ('token_name', 'decimals', 'token_short_name', 'admin_address', 'future_minting', 'white_label')
...
class ContractDetailsNeoICOSerializer(serializers.ModelSerializer):
    ...
    class Meta:
        ...
        fields = ('hard_cap', 'token_name', 'token_short_name', 'start_date', 'stop_date', 'decimals', 'rate',
...
class ContractDetailsInvestmentPoolSerializer(serializers.ModelSerializer):
    ...
    class Meta:
        ...
        fields = ('soft_cap', 'hard_cap', 'start_date', 'stop_date', 'admin_address', 'admin_percent', 'token_address',
                  'min_wei', 'max_wei', 'allow_change_dates', 'whitelist', 'investment_address', 'send_tokens_hard_cap',
                  'send_tokens_soft_cap', 'link', 'investment_tx_hash', 'balance', 'platform_as_admin', 'white_label')
        extra_kwargs = {
            'link': {
                'read_only': True
            },
...
class ContractDetailsEOSTokenSerializer(serializers.ModelSerializer):
    ...
    def to_representation(self, contract_details):
        ...
        res['token_holders'] = [
            token_holder_serializer.to_representation(th)
            for th in contract_details.contract.eostokenholder_set.order_by('id').all()
        ]
...
class ContractDetailsEOSAccountSerializer(serializers.ModelSerializer):
    ...
    class Meta:
        ...
        fields = ('owner_public_key', 'active_public_key', 'account_name', 'stake_net_value', 'stake_cpu_value',
                  'buy_ram_kbytes', 'white_label')
...
class ContractDetailsEOSICOSerializer(serializers.ModelSerializer):
    ...
    class Meta:
        ...
        fields = ('soft_cap', 'hard_cap', 'token_short_name', 'whitelist', 'is_transferable_at_once', 'start_date',
                  'stop_date', 'decimals', 'rate', 'crowdsale_address', 'min_wei', 'max_wei', 'allow_change_dates',
                  'protected_mode', 'admin_address', 'white_label')
...
class ContractDetailsEOSAirdropSerializer(serializers.ModelSerializer):
    ...
    class Meta:
        ...
        fields = ('admin_address', 'token_address', 'token_short_name', 'address_count', 'white_label')
        extra_kwargs = {
            'link': {
                'memo': True
            },
...
class ContractDetailsEOSTokenSASerializer(serializers.ModelSerializer):
    ...
    class Meta:
        ...
        fields = ('token_short_name', 'token_account', 'admin_address', 'decimals', 'maximum_supply', 'white_label')
...
class ContractDetailsGameAssetsSerializer(serializers.ModelSerializer):
    ...
    class Meta:
        ...
        fields = ('token_name', 'token_short_name', 'admin_address', 'uri', 'verification', 'verification_status',
                  'verification_date_payment', 'white_label')
        extra_kwargs = {
            'verification_status': {
                'read_only': True
...
class ContractDetailsSWAPSSerializer(serializers.ModelSerializer):
    ...
    class Meta:
        ...
        fields = ('base_address', 'quote_address', 'stop_date', 'base_limit', 'quote_limit', 'public', 'owner_address',
                  'unique_link', 'white_label')
        extra_kwargs = {'unique_link': {'read_only': True}}
    ...
    def validate(self, details):
        ...
        details['stop_date'] = datetime.datetime.strptime(details['stop_date'], '%Y-%m-%d %H:%M')
...
class ContractDetailsSWAPS2Serializer(serializers.ModelSerializer):
    ...
    class Meta:
        ...
        fields = ('base_address', 'quote_address', 'stop_date', 'base_limit', 'quote_limit', 'public', 'owner_address',
                  'unique_link', 'min_quote_wei', 'memo_contract', 'whitelist', 'whitelist_address', 'min_base_wei',
                  'broker_fee', 'broker_fee_address', 'broker_fee_base', 'broker_fee_quote', 'white_label')
        extra_kwargs = {'unique_link': {'read_only': True}, 'memo_contract': {'read_only': True}}
    ...
    def create(self, contract, contract_details):
        contract_details['memo_contract'] = '0x' + ''.join(random.choice('abcdef' + string.digits) for _ in range(64))
        kwargs = contract_details.copy()
        kwargs['contract'] = contract
        return super().create(kwargs)
...
class ContractDetailsSTOSerializer(serializers.ModelSerializer):
    ...
    class Meta:
        ...
        fields = ('asset_id', 'admin_address', 'cold_wallet_address', 'start_date', 'stop_date', 'rate', 'whitelist',
                  'soft_cap', 'hard_cap', 'min_wei', 'max_wei', 'reused_token', 'token_description', 'token_short_name',
    ...
    def validate(self, details):
        ...
        if 'decimals' in details:
            if details['decimals'] > 8 or details['decimals'] < 0:
                raise ValidationError
        details['start_date'] = datetime.datetime.strptime(details['start_date'], '%Y-%m-%d %H:%M')
        details['stop_date'] = datetime.datetime.strptime(details['stop_date'], '%Y-%m-%d %H:%M')
