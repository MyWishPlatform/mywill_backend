...
def calculate_cost_eth_token_contract(request):
    ...
    return Response({
        ...
        'WISH': str(int(eth_cost) * rate('ETH', 'WISH').value),
        'BTC': str(int(eth_cost) * rate('ETH', 'BTC').value)
    })
...
def deploy_eth_token(request):
    ...
    if contract.network.id == 1:
        ...
        if not UserSiteBalance.objects.select_for_update().filter(
                user=user, subsite__site_name=MY_WISH_URL,
                balance__gte=wish_cost).update(balance=F('balance') - wish_cost):
