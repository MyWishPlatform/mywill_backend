...
def create_waves_privkey(publicKey='', privateKey='', seed='', nonce=0):
    ...
    unhashedAddress = chr(1) + str(pw.CHAIN_ID) + crypto.hashChain(pubKey)[0:20]
...
class ContractDetailsWavesSTO(CommonDetails):
    ...
    total_supply = models.DecimalField(max_digits=MAX_WEI_DIGITS, decimal_places=0, null=True)

    ride_contract = models.ForeignKey(RideContract, null=True, default=None, on_delete=models.SET_NULL)
    ...
    def deploy(self):
        if NETWORKS[self.contract.network.name]['is_free']:
            pw.setNode(node='https://{addr}'.format(addr=NETWORKS[self.contract.network.name]['host']),
        ...
        created_address = {'publicKey': pubKey, 'privateKey': privKey, 'address': address}
    ...
    def msg_deployed(self, message):
        ...
        if self.ride_contract.id == message['contractId']:
            ...
            if self.contract.user.email:
                ...
                send_mail(
                    waves_sto_subject,
                    waves_sto_text.format(link1=network_asset.format(asset=self.asset_id,),
                                          link2=network_link.format(address=self.ride_contract.address),
