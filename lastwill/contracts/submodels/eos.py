...
from lastwill.settings import (CLEOS_TIME_COOLDOWN, CLEOS_TIME_LIMIT, EOS_ATTEMPTS_COUNT)
...
def unlock_eos_account(wallet_name, password):
    ...
    if os.system("/bin/bash -c '{command}'".format(command=lock_command)):
        raise Exception('lock command error')

    unlock_command = 'echo {password} | cleos wallet unlock -n {wallet}'.format(password=password, wallet=wallet_name)
    if os.system("/bin/bash -c '{command}'".format(command=unlock_command)):
...
class ContractDetailsEOSToken(CommonDetails):
    ...
    eos_contract = models.ForeignKey(EOSContract,
                                     null=True,
                                     default=None,
    ...
    def get_stake_params(self):
        return {'stake_net': '10.0000 EOS', 'stake_cpu': '10.0000 EOS', 'buy_ram_kbytes': 128}
    ...
    def deploy(self):
        ...
        command = [
            'cleos', '-u', eos_url, 'push', 'action', builder, 'create',
            '["{acc_name}","{max_sup} {token}"]'.format(acc_name=self.admin_address,
                                                        max_sup=max_supply,
                                                        token=self.token_short_name), '-p', acc_name, '-j'
        ]
...
class ContractDetailsEOSAccount(CommonDetails):
    ...
    def calc_cost_eos(kwargs, network):
        ...
        if network.name == 'EOS_MAINNET':
            eos_url = 'http://%s' % (str(NETWORKS[network.name]['host']))
        else:
        ...
        usdt_cost = (float(kwargs['buy_ram_kbytes']) * ram_price + float(kwargs['stake_net_value']) +
                     float(kwargs['stake_cpu_value'])) * 5 + 0.3
    ...
    def min_cost(cls):
        network = Network.objects.get(name='EOS_MAINNET')
        cost = cls.calc_cost({'stake_cpu_value': '0.64', 'stake_net_value': '0.01', 'buy_ram_kbytes': 4}, network)
        return cost

    @classmethod
    def min_cost_eos(cls):
        network = Network.objects.get(name='EOS_MAINNET')
        cost = cls.calc_cost_eos({'stake_cpu_value': '0.64', 'stake_net_value': '0.01', 'buy_ram_kbytes': 4}, network)
        return cost
    ...
    def deploy(self):
        ...
        if self.contract.network.name == 'EOS_MAINNET':
            ...
            eos_url = 'http://%s:%s' % (str(
                NETWORKS[self.contract.network.name]['host']), str(NETWORKS[self.contract.network.name]['port']))
        command = [
            'cleos', '-u', eos_url, 'system', 'newaccount', acc_name, self.account_name, self.owner_public_key,
            self.active_public_key, '--stake-net',
            str(self.stake_net_value) + ' EOS', '--stake-cpu',
            str(self.stake_cpu_value) + ' EOS', '--buy-ram-kbytes',
            str(self.buy_ram_kbytes), '--transfer', '-j'
        ]
...
class ContractDetailsEOSICO(CommonDetails):
    ...
    eos_contract_token = models.ForeignKey(EOSContract,
                                           null=True,
                                           default=None,
                                           related_name='eos_ico_details_token',
                                           on_delete=models.SET_NULL)
    eos_contract_crowdsale = models.ForeignKey(EOSContract,
                                               null=True,
                                               default=None,
    ...
    def compile(self):
        ...
        mint = ''
        for th in token_holders:
            mint = mint + ' --mint ' + '"{address} {amount}"'.format(address=th.address, amount=th.amount)
        print('mint', mint, flush=True)
        token_address = NETWORKS[self.contract.network.name]['token_address']
        dest, preproc_config = create_directory(self, sour_path='lastwill/eosio-crowdsale/*', config_name='config.h')
        command = ("/bin/bash -c 'cd {dest} && ./configure.sh "
        ...
        if os.system("/bin/bash -c 'cd {dest} && make build'".format(dest=dest)):
    ...
    def deploy(self):
        ...
        command = [
            'cleos', '-u', eos_url, 'system', 'newaccount', acc_name, self.crowdsale_address, our_public_key,
            our_public_key, '--stake-net', net, '--stake-cpu', cpu, '--buy-ram-kbytes', ram, '--transfer', '-j'
        ]
        print('command:', command, flush=True)
        tx_hash = implement_cleos_command(command)['transaction_id']
        print('tx_hash:', tx_hash, flush=True)
    ...
    def newAccount(self, message):
        ...
        if self.contract.network.name == 'EOS_MAINNET':
            ...
            # acc_name = token_address
        command = ['cleos', '-u', eos_url, 'convert', 'pack_action_data', contract_addr, 'init', str(dates)]
        print('command:', command, flush=True)
        init_data = implement_cleos_command(command)

        actions = create_eos_json(self.crowdsale_address, our_public_key, self.eos_contract_crowdsale.bytecode, abi,
                                  token_address, acc_name, max_supply, self.token_short_name,
        ...
        command = [
            ...
            acc_name,
            '-p',
            self.crowdsale_address  # do we need -p token_addres if address diff from token_address?
        ]
        print('command:', command, flush=True)
    ...
    def initialized(self, message):
        ...
        if self.contract.user.email:
            ...
            send_mail(eos_ico_subject, eos_ico_message.format(network_name=network_name), DEFAULT_FROM_EMAIL,
...
class ContractDetailsEOSAirdrop(CommonDetails):
    ...
    eos_contract = models.ForeignKey(EOSContract, null=True, default=None, on_delete=models.SET_NULL)
    token_short_name = models.CharField(max_length=64)
    ...
    def deploy(self):
        ...
        unlock_eos_account(wallet_name, password)
        command = ['cleos', '-u', eos_url, 'get', 'table', self.token_address, self.token_short_name, 'stat']
        print('command', command)
        ...
        command = [
            'cleos', '-u', eos_url, 'push', 'action', airdrop_address, 'create',
    ...
    def airdrop(self, message):
        ...
        for js in message['airdroppedAddresses']:
            ...
            if addr is None and message['status'] == 'COMMITTED':
                old_state = 'added'
                addr = EOSAirdropAddress.objects.filter(address=address,
                                                        amount=amount,
                                                        contract=self.contract,
                                                        active=True,
        ...
        elif self.contract.airdropaddress_set.filter(state__in=('added', 'processing'), active=True).count() == 0:
    ...
    def msg_deployed(self, message):
        ...
        self.contract.save()
        if self.contract.user.email:
            send_mail(eos_airdrop_subject,
                      eos_airdrop_message.format(network_name=network_name, hash=self.eos_contract.tx_hash),
